...
$.fn.fill_apbyname = function(btsname,selected){
	$(this).html("");
	thisap = $(this);
	thisap.append('<option value="Belum ada AP" selected=selected>Belum ada AP</option>');
	$.ajax({
		url:'/paps/get_name_by_btsname',
		type:'post',
		data:{btsname:btsname},
		dataType:'json',
...
initbaupload = function(){
	...
	new AjaxUpload(btnUpload, {
		...
		if (! (ext && /^(jpg|png|jpeg|gif|pdf|doc|docx|xls|xlsx|odt|ods)$/.test(ext))){
