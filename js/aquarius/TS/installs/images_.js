(function($){
	...
	$("#saveimage").click(function(){
		$.post(thisdomain+'install_images/add',{install_site_id:$("#workplace").attr("install_site_id"),user_name:$("#createuser").val(),path:$("#path_image").val(),description:$("#description_image").val()}).done(function(data){
			$("#install_image tbody").append('<tr class="rImage" image_id='+data+' image_path='+$("#path_image").val()+'><td class="image_path"><a class="fancybox" rel="group" href="'+baseurl+'media/installs/'+$("#path_image").val()+'"><img src="'+baseurl+'media/installs/'+$("#path_image").val()+'" class="img-polaroid" width=50 height=38 /></a></td><td class="info image_name"><a class="fancybox" rel="group" href="'+baseurl+'media/installs/'+$("#path_image").val()+'"></a> <span>'+$("#path_image").val()+'</span></td><td class="image_description">'+$("#description_image").val()+'</td><td><a><span class="icon-trash removeinstallimage pointer" ></span></a><a><span class="icon-arrow-up switchup pointer"></span></a><a><span class="icon-arrow-down switchdown pointer"></span></a></td></tr>');
			update_rowcount($("#total_image"),$("#install_image tbody tr"));
