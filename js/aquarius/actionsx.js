$(document).ready(function(){
    var ipaddr = '192.168.3.245';
    var baseurl = 'http://'+ipaddr+'/db_teknis/';
    var thisdomain = 'http://db_teknis/';
    ...
    $('.btsdistance_remove').click(function(){
		...
		$.post(thisdomain+"index.php/adm/btsdistance_remove",{btsdistance_id:$(this).attr('btsdistance_id')}).done(function(data){alert(data);}).fail(function(data){alert('gagal');});
        return false;
	});
	...
	$('.otherclient_remove').click(function(){
		$(this).parent().parent().parent().fadeOut(200);
		$.post(thisdomain+"index.php/adm/survey_removeotherclent",{id:$(this).attr('id')}).done(function(data){}).fail(function(){alert('gagal');});
	});
	...
	removepetugassurvey = function(){
            $(this).parent().parent().parent().fadeOut(200);
            $.post(baseurl+"index.php/adm/survey_removeofficer",{survey_id:$(this).attr('survey_id'),user_id:$(this).attr('user_id')});
	}
   ...
   $("#saveSurveySite").click(function(){
	   $.post(thisdomain+'index.php/adm/addsite',{survey_request_id:$(this).attr('survey_id'),address:$('#site_address').val(),city:$('#site_city').val(),phone_area:$('#site_phone_area').val(),phone:$('#site_phone').val(),pic_name:$('#site_pic_name').val(),pic_position:$('#site_pic_position').val(),pic_email:$('#pic_email').val(),elevation_ground:'',elevation_rooftop:'',bearing:'',leg_course:'',leg_dist:'',amsl:'',agl:''}).done(function(data){
		   createsurveysite($(this).attr('survey_id'),data);
		   }).fail(function(){alert('gagal cak');});
	   addsurveysite();
	});
	
     $(".link_navRemSurveySite").click(function(){
            $(this).parent().parent().parent().fadeOut(200);
            $.post(baseurl+"index.php/adm/survey_removesite",{survey_id:$(this).attr('survey_id'),site_id:$(this).attr('site_id')});
        return false;
    });
    ...
    $('.device_remove').click(function(){
		$(this).parent().parent().parent().fadeOut(200);
        $.post(thisdomain+"index.php/adm/survey_removedevice",{device_id:$(this).attr('device_id')}).done(function(data){}).fail(function(){alert('gagal');});
        return false;
	});
