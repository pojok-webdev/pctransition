...
echo form_open_multipart('back_end/upload_banner');
...
echo '<tr>';
echo '<td>' . $f . '</td>';
echo '<td><a class="fance" href="' . base_url() . 'media/banners/' . $f . '">' . $this->lang->line('preview') . '</a></td>';
echo '<td>' . anchor('back_end/entry_banner/type/edit/id/' . $identifier_value . '/image/' . $f . '/page/0',$this->lang->line('use')) . '</td>';
echo '<td>' . anchor('back_end/remove_banner/identifier/' . $identifier . '/identifier_value/' . $identifier_value . '/pic/' . $f . '/image/' . $ori_image,$this->lang->line('delete'),'class="text_link"') . '</td>';
echo '</tr>';
}
echo '</table>';
echo '</div>';
