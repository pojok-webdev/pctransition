		...
		<h5>
			...
			<div class="radio">
				...
				<?php foreach($userbranches as $branch){?>
					<label class="checkbox-inline"><input type="checkbox" value="<?php echo $branch;?>" class="suspect-branches"  <?php echo branchischecked($branch,$this->uri->segment(7)) ;?>><?php echo getbranch($branch)?></label>
				<?php }?>
<!--
				<label class="checkbox-inline"><input type="checkbox" value="1" class="suspect-branches" <?php echo $sbychecked;?>>Surabaya</label>
				<label class="checkbox-inline"><input type="checkbox" value="2" class="suspect-branches" <?php echo $jktchecked;?>>Jakarta</label>
				<label class="checkbox-inline "><input type="checkbox" value="4" class="suspect-branches" <?php echo $blichecked;?>>Bali</label>
				<label class="checkbox-inline"><input type="checkbox" value="3" class="suspect-branches" <?php echo $mlgchecked;?>>Malang</label>-->
				<br />
			</div>		

			<br />
			<span id="amsby">
				<?php //foreach(getambybranch($branches,$dt1,$dt2) as $am){
					foreach($users as $am){
					if(in_array($am,$ams)){
		...
		<script type="text/javascript">
			...
			orderlink = function(orderby,branch){
				...
				window.location.href = baseurl+"rpt/suspectreport/"+orderby+"/"+rorder+"/"+$("#dt1").attr("datetime")+"/"+$("#dt2").attr("datetime")+"/"+branch+"/"+amarr.join("-");
			}
				...
				$("#btnfilter").click(function(){
					...
					if(allchecked){
						window.location.href = baseurl+"rpt/suspectreport/"+urlsegment(3)+"/"+order+"/"+$("#dt1").attr("datetime")+"/"+$("#dt2").attr("datetime")+"/"+branch+"/"+amarr.join("-");
					}else{
