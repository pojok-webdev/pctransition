	...
	<body>
		...
		<h5>
			...
			<div class="radio">
				...
				<?php foreach($userbranches as $branch){?>
					<label class="checkbox-inline"><input type="checkbox" value="<?php echo $branch;?>" class="survey-branches"  <?php echo branchischecked($branch,$this->uri->segment(7)) ;?>><?php echo getbranch($branch)?></label>
				<?php }?>
				<!--			
				<label class="checkbox-inline"><input type="checkbox" value="1" class="survey-branches" <?php echo $sbychecked;?>>Surabaya</label>
				<label class="checkbox-inline"><input type="checkbox" value="2" class="survey-branches" <?php echo $jktchecked;?>>Jakarta</label>
				<label class="checkbox-inline "><input type="checkbox" value="4" class="survey-branches" <?php echo $blichecked;?>>Bali</label>
				<label class="checkbox-inline"><input type="checkbox" value="3" class="survey-branches" <?php echo $mlgchecked;?>>Malang</label>
				-->
				<br />
			</div>		
			...
			<table id="rpt" class="table">
				...
				<tbody>
					...
					<tr>
						...
						<td><?php echo $survey->create_date;?></td>
						<td>
							<?php echo $survey->name;?>
						</td>
						<td>
							<?php echo $survey->address;?>
						</td>
						<td>
							<?php echo $survey->hunter;?>
						</td>
						<td>
							<?php echo $survey->brn;?>
						</td>
		...
		<script type="text/javascript">
			...
			orderlink = function(orderby,branch){
				branch ='';
				$.each($(".survey-branches:checked"),function(){
					branch+=$(this).val();
				})
					am = "";
					amarr = [];
				...
				window.location.href = baseurl+"rpt/installreport/"+orderby+"/"+rorder+"/"+$("#dt1").attr("datetime")+"/"+$("#dt2").attr("datetime")+"/"+branch+"/"+amarr.join("-");
			}
				...
				$("#btnfilter").click(function(){
					...
					if(allchecked){
						window.location.href = baseurl+"rpt/installreport/"+urlsegment(3)+"/"+order+"/"+$("#dt1").attr("datetime")+"/"+$("#dt2").attr("datetime")+"/"+branch+"/"+amarr.join("-");
					}else{
