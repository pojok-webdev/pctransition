...
class Auth
{
	...
	function validate_password($password){
		...
		$user = new User();
		$user->where('id',$this->CI->session->userdata['id'])->get();
	...
	function create_temp_user($username,$password,$email,$fullname,$address,$phone){
		$qry = $this->CI->db->where('username', $username)
							->or_where('email', $email)
							->get('users');
		...
		$query = 'insert into registrars (username,password,salt,email,fullname,address,phone) ';
		$query.= 'values ';
		$query.= '("' . $data['username'];
		$query .= '","' . $data['password'];
		$query .= '","' . $data['salt'];
